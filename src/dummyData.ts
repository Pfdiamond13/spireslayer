...
interface Card {
  ...
  energy: number;
  type: CardType;
  action: (gameProps: GameProps) => GameProps;
}

const defAction = (gameProps: GameProps) => gameProps

const data: { cards: Card[] } = {
  cards: [
    { name: 'Strike', dmg: 6, block: 0, energy: 1, type: CardType.Attack, action: defAction},
